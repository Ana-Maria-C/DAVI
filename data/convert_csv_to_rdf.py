...
def sanitize_uri(text):
    ...
    return urllib.parse.quote(str(text).replace(" ", "_").replace(":", "").replace("(", "").replace(")", "").replace("'", ""))
...
def convert_csv_to_rdf(csv_path, output_path):
    ...
    for _, row in df.iterrows():
        ...
        if movie_id not in processed_movies:
            ...
            g.add((movie_uri, MOVIELENS.movieId, Literal(movie_id, datatype=XSD.integer)))
        ...
        g.add((rating_uri, MOVIELENS.ratingValue, Literal(rating_val, datatype=XSD.float)))
